...
namespace
{
  ...
  const error_code ok_really_its_okay_no_name_conflict_here;
...
//--------------------------------------------------------------------------------------//
  ...
  bool // true if ok_really_its_okay_no_name_conflict_here
...
#   endif
    ...
    result = max;
    return ok_really_its_okay_no_name_conflict_here;
  }
...
#   ifdef BOOST_POSIX_API
    std::free(buffer);
    buffer = 0;
    if (handle == 0)return ok_really_its_okay_no_name_conflict_here;
